...
class QNetwork(torch.nn.Module):
    def __init__(self, state_size, action_size):
        ...
        self.lp = dict(
            # todo: 사이즈 맞춰서 len 변경
            history_len=30, step_len=5, seq_len=29, batch_size=128, criterion=torch.nn.MSELoss(),
            max_epochs=15, n_features=1, hidden_size=50, num_layers=1, dropout=0.8, learning_rate=0.01
    ...
    def forward(self, x1, x2):
        ...
        x1, _ = self.lstm(x1)
        x2 = self.cnn(torch.stack([x2])) # 배치 1개
        # Fusion model
